...
def generate(p):
    prompt = f'''Write B2B procurement website copy for the product "{p['name']}" (brand: {p['brand'] or 'n/a'}, origin: {p['origin'] or 'n/a'}, category: {p['category']}), sold WHOLESALE by SOLA Medical Supply to clinics, spas, resellers and distributors.
This is sourcing/procurement information for professional buyers. It is NOT medical advice and NOT consumer marketing.
Hard rules: do NOT state medical efficacy, dosing, treatment outcomes, prices, stock levels, certifications, regulatory approvals, or claim SOLA is an authorised/official distributor. No patient-facing claims.
...
def page(p, a):
    ...
    wa_link = f"{WA}?text=" + urllib.parse.quote(f"Hello SOLA Medical Supply, please send a wholesale quote for {p['name']} (destination country: )")
    ld = json.dumps({"@context": "https://schema.org", "@graph": [
        {"@type": "Product", "name": p["name"], "image": img_abs, "category": p["category"],
         "brand": {"@type": "Brand", "name": p["brand"] or "SOLA Medical Supply"},
